"""
Publication Acronym Reference Abbreviation
https://github.com/JabRef/abbrv.jabref.org/blob/master/journals/journal_abbreviations_ieee.txt
#Content is based on IEEEfull.bib and IEEEabrv.bib as of 2016-03-25
#A recent list of abbreviations is available at https://www.ieee.org/documents/trans_journal_names.pdf

Parsable:
    https://www.ieee.org/documents/ieee_conference_acronyms.pdf
    http://librarybestbets.fairfield.edu/citationguides/ieee#AbbreviationsforIEEEPublications
"""

JabRef_list = """
Canadian Journal of Electrical and Computer Engineering = Canadian J. Elect. Comput. Eng.
China Communications Magazine = China Commun.
Computer = Computer
Proceedings of the {IEEE} = Proc. {IEEE}
Today's Engineer = Today's Engineer
{IEEE/ACM} Transactions on Audio, Speech, and Language Processing = {IEEE/ACM} Trans. Audio, Speech, Language Process.
{IEEE/ACM} Transactions on Computational Biology and Bioinformatics = {IEEE/ACM} Trans. Comput. Biol. Bioinformatics
{IEEE/ACM} Transactions on Networking = {IEEE/ACM} Trans. Netw.
{IEEE/ASME} Journal of Microelectromechanical Systems = J. Microelectromech. Syst.
{IEEE/ASME} Transactions on Mechatronics = {IEEE/ASME} Trans. Mechatronics
{IEEE/CAA} Journal of Automatica Sinica = {IEEE/CAA} J. Autom. Sinica
{IEEE/ECS} Electrochemical and Solid-State Letters = {IEEE/ECS} Electrochem. Solid-State Lett.
{IEEE/OSA} Journal of Display Technology = {IEEE/OSA} J. Display Technol.
{IEEE/OSA} Journal of Lightwave Technology = J. Lightw. Technol.
{IEEE/TMS} Journal of Electronic Materials = {IEEE/TMS} J. Electron. Mater.
{IEEE} Access = {IEEE} Access
{IEEE} Aerospace and Electronics Systems Magazine = {IEEE} Aerosp. Electron. Syst. Mag.
{IEEE} Annals of the History of Computing = {IEEE} Ann. Hist. Comput.
{IEEE} Antennas and Propagation Magazine = {IEEE} Antennas Propag. Mag.
{IEEE} Antennas and Wireless Propagation Letters = {IEEE} Antennas Wireless Propag. Lett.
{IEEE} Circuits and Devices Magazine = {IEEE} Circuits Devices Mag.
{IEEE} Circuits and Systems Magazine = {IEEE} Circuits Syst. Mag.
{IEEE} Communications Letters = {IEEE} Commun. Lett.
{IEEE} Communications Magazine = {IEEE} Commun. Mag.
{IEEE} Communications Society Magazine = {IEEE} Commun. Soc. Mag.
{IEEE} Communications Surveys and Tutorials = {IEEE} Commun. Surveys Tuts.
{IEEE} Computational Intelligence Magazine = {IEEE} Comput. Intell. Mag.
{IEEE} Computational Science and Engineering Magazine = {IEEE} Comput. Sci. Eng. Mag.
{IEEE} Computer Applications in Power = {IEEE} Comput. Appl. Power
{IEEE} Computer Architecture Letters = {IEEE} Comput. Archit. Lett.
{IEEE} Computer Graphics and Applications = {IEEE} Comput. Graph. Appl.
{IEEE} Computing in Science and Engineering = {IEEE} Comput. Sci. Eng.
{IEEE} Concurrency = {IEEE} Concurrency
{IEEE} Control Systems Magazine = {IEEE} Control Syst. Mag.
{IEEE} Design \&\ Test = {IEEE} Des. Test
{IEEE} Design and Test of Computers = {IEEE} Des. Test. Comput.
{IEEE} Distributed Systems Online = {IEEE} Distrib. Syst. Online
{IEEE} Electrical Insulation Magazine = {IEEE} Electr. Insul. Mag.
{IEEE} Electrification Magazine = {IEEE} Electrific. Mag.
{IEEE} ElectroTechnology Review = {IEEE} ElectroTechnol. Rev.
{IEEE} Electromagnetic Compatibility Magazine = {IEEE} Electromagn. Compat.
{IEEE} Electron Device Letters = {IEEE} Electron Device Lett.
{IEEE} Embedded Systems Letters = {IEEE} Embedded Syst. Lett.
{IEEE} Engineering Management Review = {IEEE} Eng. Manag. Rev.
{IEEE} Engineering in Medicine and Biology Magazine = {IEEE} Eng. Med. Biol. Mag.
{IEEE} Expert = {IEEE} Expert
{IEEE} Geoscience and Remote Sensing Letters = {IEEE} Geosci. Remote Sens. Lett.
{IEEE} Geoscience and Remote Sensing Magazine = {IEEE} Geosci. Remote Sens. Mag.
{IEEE} Industrial Electronics Magazine = {IEEE} Ind. Electron. Mag.
{IEEE} Industry Applications Magazine = {IEEE} Ind. Appl. Mag.
{IEEE} Instrumentation and Measurement Magazine = {IEEE} Instrum. Meas. Mag.
{IEEE} Intelligent Systems = {IEEE} Intell. Syst.
{IEEE} Intelligent Transportation Systems Magazine = {IEEE} Intell. Transp. Syst. Mag.
{IEEE} Internet Computing = {IEEE} Internet Comput.
{IEEE} Internet of Things Journal = {IEEE} Internet Things J.
{IEEE} Journal of Biomedical and Health Informatics = {IEEE} J. Biomed. Health Inform.
{IEEE} Journal of Electron Devices Society = {IEEE} J. Electron Devices Soc.
{IEEE} Journal of Microelectromechanical Systems = J. Microelectromech. Syst.
{IEEE} Journal of Oceanic Engineering = {IEEE} J. Ocean. Eng.
{IEEE} Journal of Optical Communications and Networking = {IEEE} J. Opt. Commun. Netw.
{IEEE} Journal of Photovoltaics = {IEEE} J. Photovolt.
{IEEE} Journal of Product Safety Engineering = {IEEE} J. Product Safety Eng.
{IEEE} Journal of Quantum Electronics = {IEEE} J. Quantum Electron.
{IEEE} Journal of Robotics and Automation = {IEEE} J. Robot. Autom.
{IEEE} Journal of Selected Topics in Applied Earth Observations and Remote Sensing = {IEEE} J. Sel. Topics Appl. Earth Observ. Remote Sens.
{IEEE} Journal of Selected Topics in Quantum Electronics = {IEEE} J. Sel. Topics Quantum Electron.
{IEEE} Journal of Selected Topics in Signal Processing = {IEEE} J. Sel. Topics Signal Process.
{IEEE} Journal of Solid-State Circuits = {IEEE} J. Solid-State Circuits
{IEEE} Journal of Translational Engineering in Health and Medicine = {IEEE} J. Transl. Eng. Health Med.
{IEEE} Journal on Exploratory Solid-State Computational Devices and Circuits = {IEEE} J. Explor. Solid-State Computat. Devices Circuits
{IEEE} Journal on Multiscale and Multiphysics Computational Techniques = {IEEE} J. Multiscale and Multiphys. Comput. Techn.
{IEEE} Journal on Selected Areas in Communications = {IEEE} J. Sel. Areas Commun.
{IEEE} Journal on Technology in Computer Aided Design = {IEEE} J. Technol. Comput. Aided Design
{IEEE} Life Sciences Letters = {IEEE} Life Sci. Lett.
{IEEE} Magnetics Letters = {IEEE} Magn. Lett.
{IEEE} Micro = {IEEE} Micro
{IEEE} Microwave Magazine = {IEEE} Microw. Mag.
{IEEE} Microwave and Guided Wave Letters = {IEEE} Microw. Guided Wave Lett.
{IEEE} Microwave and Wireless Components Letters = {IEEE} Microw. Wireless Compon. Lett.
{IEEE} Multimedia = {IEEE} Multimedia
{IEEE} Nanotechnology Express = {IEEE} Nanotechnol. Express
{IEEE} Nanotechnology Magazine = {IEEE} Nanotechnol. Mag.
{IEEE} Network = {IEEE} Netw.
{IEEE} Personal Communications Magazine = {IEEE} Personal Commun. Mag.
{IEEE} Pervasive Computing = {IEEE} Pervasive Comput.
{IEEE} Photonics Journal = {IEEE} Photon. J.
{IEEE} Photonics Technology Letters = {IEEE} Photon. Technol. Lett.
{IEEE} Potentials = {IEEE} Potentials
{IEEE} Power Electronics Letters = {IEEE} Power Electron. Lett.
{IEEE} Power Electronics Magazine = {IEEE} Power Electron. Mag.
{IEEE} Power Engineering Review = {IEEE} Power Eng. Rev.
{IEEE} Power and Energy Magazine = {IEEE} Power Energy Mag.
{IEEE} Power and Energy Technology Systems Journal = {IEEE} Power Energy Technol. Syst. J.
{IEEE} Pulse = {IEEE} Pulse
{IEEE} Reviews in Biomedical Engineering = {IEEE} Rev. Biomed. Eng.
{IEEE} Revista Iberoamericana de Technolog{\'{i}}as del Aprendizaje = {IEEE} Revista Iberoamericana de Technolog{\'{i}}as del Aprendizaje
{IEEE} Robotics and Automation Letters = {IEEE} Robot. Autom. Lett.
{IEEE} Robotics and Automation Magazine = {IEEE} Robot. Autom. Mag.
{IEEE} Security and Privacy = {IEEE} Security Privacy
{IEEE} Sensors Journal = {IEEE} Sensors J.
{IEEE} Signal Processing Letters = {IEEE} Signal Process. Lett.
{IEEE} Signal Processing Magazine = {IEEE} Signal Process. Mag.
{IEEE} Software = {IEEE} Softw.
{IEEE} Solid-State Circuits Magazine = {IEEE} Solid-State Circuits Mag.
{IEEE} Spectrum = {IEEE} Spectr.
{IEEE} Sustainable Computing = {IEEE} Sustain. Comput.
{IEEE} Systems Journal = {IEEE} Syst. J.
{IEEE} Systems, Man, and Cybernetics Magazine = {IEEE} Syst., Man, Cybern. Mag.
{IEEE} Technology and Society Magazine = {IEEE} Technol. Soc. Mag.
{IEEE} Transactions on Acoustics, Speech, and Signal Processing = {IEEE} Trans. Acoust., Speech, Signal Process.
{IEEE} Transactions on Advanced Packaging = {IEEE} Trans. Adv. Packag.
{IEEE} Transactions on Aeronautical and Navigational Electronics = {IEEE} Trans. Aeronaut. Navig. Electron.
{IEEE} Transactions on Aerospace = {IEEE} Trans. Aerosp.
{IEEE} Transactions on Aerospace and Electronic Systems = {IEEE} Trans. Aerosp. Electron. Syst.
{IEEE} Transactions on Aerospace and Navigational Electronics = {IEEE} Trans. Aerosp. Navig. Electron.
{IEEE} Transactions on Affective Computing = {IEEE} Trans. Affect. Comput.
{IEEE} Transactions on Airborne Electronics = {IEEE} Trans. Airborne Electron.
{IEEE} Transactions on Antennas and Propagation = {IEEE} Trans. Antennas Propag.
{IEEE} Transactions on Applications and Industry = {IEEE} Trans. Appl. Ind.
{IEEE} Transactions on Applied Superconductivity = {IEEE} Trans. Appl. Supercond.
{IEEE} Transactions on Audio = {IEEE} Trans. Audio
{IEEE} Transactions on Audio and Electroacoustics = {IEEE} Trans. Audio Electroacoust.
{IEEE} Transactions on Audio, Speech, and Language Processing = {IEEE} Audio, Speech, Language Process.
{IEEE} Transactions on Automatic Control = {IEEE} Trans. Autom. Control
{IEEE} Transactions on Automation Science and Engineering = {IEEE} Trans. Autom. Sci. Eng.
{IEEE} Transactions on Autonomous Mental Development = {IEEE} Trans. Auton. Mental Develop.
{IEEE} Transactions on Big Data = {IEEE} Trans. Big Data
{IEEE} Transactions on Bio-Medical Electronics = {IEEE} Trans. Bio-Med. Electron.
{IEEE} Transactions on Biomedical Circuits and Systems = {IEEE} Trans. Biomed. Circuits Syst.
{IEEE} Transactions on Biomedical Engineering = {IEEE} Trans. Biomed. Eng.
{IEEE} Transactions on Broadcast and Television Receivers = {IEEE} Trans. Broadcast Television Receivers
{IEEE} Transactions on Broadcasting = {IEEE} Trans. Broadcast.
{IEEE} Transactions on Circuit Theory = {IEEE} Trans. Circuit Theory
{IEEE} Transactions on Circuits and Systems = {IEEE} Trans. Circuits Syst.
{IEEE} Transactions on Circuits and Systems for Video Technology = {IEEE} Trans. Circuits Syst. Video Technol.
{IEEE} Transactions on Circuits and Systems---Part {II}: Analog and Digital Signal Processing = {IEEE} Trans. Circuits Syst. {II}
{IEEE} Transactions on Circuits and Systems---Part {II}: Express Briefs = {IEEE} Trans. Circuits Syst. {II}
{IEEE} Transactions on Circuits and Systems---Part {I}: Fundamental Theory and Applications = {IEEE} Trans. Circuits Syst. {I}
{IEEE} Transactions on Circuits and Systems---Part {I}: Regular Papers = {IEEE} Trans. Circuits Syst. {I}
{IEEE} Transactions on Cloud Computing = {IEEE} Trans. on Cloud Comput.
{IEEE} Transactions on Cognitive Communications and Networking = {IEEE} Trans. on Cogn. Commun. Netw.
{IEEE} Transactions on Cognitive and Developmental Systems = {IEEE} Trans. Cogn. Develop. Syst.
{IEEE} Transactions on Communication Technology = {IEEE} Trans. Commun. Technol.
{IEEE} Transactions on Communications = {IEEE} Trans. Commun.
{IEEE} Transactions on Component Parts = {IEEE} Trans. Compon. Parts
{IEEE} Transactions on Components and Packaging Technologies = {IEEE} Trans. Compon. Packag. Technol.
{IEEE} Transactions on Components and Packaging Technology = {IEEE} Trans. Compon. Packag. Technol.
{IEEE} Transactions on Components, Hybrids and Manufacturing Technology = {IEEE} Trans. Compon., Hybrids, Manuf. Technol.
{IEEE} Transactions on Components, Packaging and Manufacturing Technology = {IEEE} Trans. Compon. Packag. Manuf. Technol.
{IEEE} Transactions on Components, Packaging and Manufacturing Technology---Part {A} = {IEEE} Trans. Compon., Packag., Manuf. Technol. {A}
{IEEE} Transactions on Components, Packaging and Manufacturing Technology---Part {B}: Advanced Packaging = {IEEE} Trans. Compon., Packag., Manuf. Technol. {B}
{IEEE} Transactions on Components, Packaging and Manufacturing Technology---Part {C}: Manufacturing = {IEEE} Trans. Compon., Packag., Manuf. Technol. {C}
{IEEE} Transactions on Computational Intelligence and {AI} in Games = {IEEE} Trans. Comput. Intell. {AI} in Games
{IEEE} Transactions on Computational Social Systems = {IEEE} Trans. Comput. Social Syst.
{IEEE} Transactions on Computer-Aided Design of Integrated Circuits and Systems = {IEEE} Trans. Comput.-Aided Design Integr. Circuits Syst.
{IEEE} Transactions on Computers = {IEEE} Trans. Comput.
{IEEE} Transactions on Consumer Electronics = {IEEE} Trans. Consum. Electron.
{IEEE} Transactions on Control Systems Technology = {IEEE} Trans. Control Syst. Technol.
{IEEE} Transactions on Control of Network Systems = {IEEE} Trans. Control Netw. Syst.
{IEEE} Transactions on Cybernetics = {IEEE} Trans. Cybern.
{IEEE} Transactions on Dependable and Secure Computing = {IEEE} Trans. Depend. Sec. Comput.
{IEEE} Transactions on Device and Materials Reliability = {IEEE} Trans. Device Mater. Rel.
{IEEE} Transactions on Dielectrics and Electrical Insulation = {IEEE} Trans. Dielectr. Electr. Insul.
{IEEE} Transactions on Education = {IEEE} Trans. Educ.
{IEEE} Transactions on Electrical Insulation = {IEEE} Trans. Electr. Insul.
{IEEE} Transactions on Electromagnetic Compatibility = {IEEE} Trans. Electromagn. Compat.
{IEEE} Transactions on Electron Devices = {IEEE} Trans. Electron Devices
{IEEE} Transactions on Electronic Computers = {IEEE} Trans. Electron. Comput.
{IEEE} Transactions on Electronics Packaging Manufacturing = {IEEE} Trans. Electron. Packag. Manuf.
{IEEE} Transactions on Emerging Topics in Computing = {IEEE} Trans. Emerg. Topics Comput.
{IEEE} Transactions on Emerging and Selected Topics in Circuits and Systems = {IEEE} Trans. Emerg. Sel. Topics Circuits Syst.
{IEEE} Transactions on Emerging and Selected Topics in Power Electronics = {IEEE} Trans. Emerg. Sel. Topics Power Electron.
{IEEE} Transactions on Energy Conversion = {IEEE} Trans. Energy Convers.
{IEEE} Transactions on Engineering Management = {IEEE} Trans. Eng. Manag.
{IEEE} Transactions on Engineering Writing and Speech = {IEEE} Trans. Eng. Writing Speech
{IEEE} Transactions on Evolutionary Computation = {IEEE} Trans. Evol. Comput.
{IEEE} Transactions on Fuzzy Systems = {IEEE} Trans. Fuzzy Syst.
{IEEE} Transactions on Geoscience Electronics = {IEEE} Trans. Geosci. Electron.
{IEEE} Transactions on Geoscience and Remote Sensing = {IEEE} Trans. Geosci. Remote Sens.
{IEEE} Transactions on Haptics = {IEEE} Trans. Haptics
{IEEE} Transactions on Human Factors in Electronics = {IEEE} Trans. Hum. Factors Electron.
{IEEE} Transactions on Human-Machine Systems = {IEEE} Trans. Human-Mach. Syst.
{IEEE} Transactions on Image Processing = {IEEE} Trans. Image Process.
{IEEE} Transactions on Industrial Electronics = {IEEE} Trans. Ind. Electron.
{IEEE} Transactions on Industrial Electronics and Control Instrumentation = {IEEE} Trans. Ind. Electron. Contr. Instrum.
{IEEE} Transactions on Industrial Informatics = {IEEE} Trans. Ind. Informat.
{IEEE} Transactions on Industry Applications = {IEEE} Trans. Ind. Appl.
{IEEE} Transactions on Industry and General Applications = {IEEE} Trans. Ind. Gen. Appl.
{IEEE} Transactions on Information Forensics and Security = {IEEE} Trans. Inf. Forensics Security
{IEEE} Transactions on Information Technology in Biomedicine = {IEEE} Trans. Inf. Technol. Biomed.
{IEEE} Transactions on Information Theory = {IEEE} Trans. Inf. Theory
{IEEE} Transactions on Instrumentation and Measurement = {IEEE} Trans. Instrum. Meas.
{IEEE} Transactions on Intelligent Transportation Systems = {IEEE} Trans. Intell. Transp. Syst.
{IEEE} Transactions on Intelligent Vehicles = {IEEE} Trans. Intell. Veh.
{IEEE} Transactions on Knowledge and Data Engineering = {IEEE} Trans. Knowl. Data Eng.
{IEEE} Transactions on Learning Technologies = {IEEE} Trans. Learn. Technol.
{IEEE} Transactions on Magnetics = {IEEE} Trans. Magn.
{IEEE} Transactions on Man-Machine Systems = {IEEE} Trans. Man-Mach. Syst.
{IEEE} Transactions on Manufacturing Technology = {IEEE} Trans. Manuf. Technol.
{IEEE} Transactions on Medical Electronics = {IEEE} Trans. Med. Electron.
{IEEE} Transactions on Medical Imaging = {IEEE} Trans. Med. Imag.
{IEEE} Transactions on Microwave Theory and Techniques = {IEEE} Trans. Microw. Theory Techn.
{IEEE} Transactions on Military Electronics = {IEEE} Trans. Mil. Electron.
{IEEE} Transactions on Mobile Computing = {IEEE} Trans. Mobile Comput.
{IEEE} Transactions on Molecular, Biological and Multi-Scale Communications = {IEEE} Trans. Mol. Biol. Multi-Scale Commun.
{IEEE} Transactions on Multi-Scale Computing Systems = {IEEE} Trans. Multi-Scale Comput. Syst.
{IEEE} Transactions on Multimedia = {IEEE} Trans. Multimedia
{IEEE} Transactions on NanoBioscience = {IEEE} Trans. Nanobiosci.
{IEEE} Transactions on Nanotechnology = {IEEE} Trans. Nanotechnol.
{IEEE} Transactions on Network Science and Engineering = {IEEE} Trans. Netw. Sci. Eng.
{IEEE} Transactions on Network and Service Management = {IEEE} Trans. Netw. Service Manag.
{IEEE} Transactions on Neural Networks = {IEEE} Trans. Neural Netw.
{IEEE} Transactions on Neural Networks and Learning Systems = {IEEE} Trans. Neural Netw. Learn. Syst.
{IEEE} Transactions on Neural Systems and Rehabilitation Engineering = {IEEE} Trans. Neural Syst. Rehabil. Eng.
{IEEE} Transactions on Nuclear Science = {IEEE} Trans. Nucl. Sci.
{IEEE} Transactions on Parallel and Distributed Systems = {IEEE} Trans. Parallel Distrib. Syst.
{IEEE} Transactions on Parts, Hybrids and Packaging = {IEEE} Trans. Parts, Hybrids, Packag.
{IEEE} Transactions on Parts, Materials and Packaging = {IEEE} Trans. Parts, Mater., Packag.
{IEEE} Transactions on Pattern Analysis and Machine Intelligence = {IEEE} Trans. Pattern Anal. Mach. Intell.
{IEEE} Transactions on Plasma Science = {IEEE} Trans. Plasma Sci.
{IEEE} Transactions on Power Apparatus and Systems = {IEEE} Trans. Power App. Syst.
{IEEE} Transactions on Power Delivery = {IEEE} Trans. Power Del.
{IEEE} Transactions on Power Electronics = {IEEE} Trans. Power Electron.
{IEEE} Transactions on Power Systems = {IEEE} Trans. Power Syst.
{IEEE} Transactions on Professional Communication = {IEEE} Trans. Prof. Commun.
{IEEE} Transactions on Radio Frequency Interference = {IEEE} Trans. Radio Freq. Interference
{IEEE} Transactions on Rehabilitation Engineering = {IEEE} Trans. Rehabil. Eng.
{IEEE} Transactions on Reliability = {IEEE} Trans. Rel.
{IEEE} Transactions on Robotics = {IEEE} Trans. Robot.
{IEEE} Transactions on Robotics and Automation = {IEEE} Trans. Robot. Autom.
{IEEE} Transactions on Semiconductor Manufacturing = {IEEE} Trans. Semicond. Manuf.
{IEEE} Transactions on Services Computing = {IEEE} Trans. Serv. Comput.
{IEEE} Transactions on Signal Processing = {IEEE} Trans. Signal Process.
{IEEE} Transactions on Signal and Information Processing over Networks = {IEEE} Trans. Signal Inf. Process. Netw.
{IEEE} Transactions on Smart Grid = {IEEE} Trans. Smart Grid
{IEEE} Transactions on Software Engineering = {IEEE} Trans. Softw. Eng.
{IEEE} Transactions on Sonics and Ultrasonics = {IEEE} Trans. Sonics Ultrason.
{IEEE} Transactions on Speech and Audio Processing = {IEEE} Trans. Speech Audio Process.
{IEEE} Transactions on Sustainable Energy = {IEEE} Trans. Sustain. Energy
{IEEE} Transactions on Systems Science and Cybernetics = {IEEE} Trans. Syst. Sci. Cybern.
{IEEE} Transactions on Systems, Man, and Cybernetics = {IEEE} Trans. Syst., Man, Cybern.
{IEEE} Transactions on Systems, Man, and Cybernetics---Part {A}: Systems and Humans = {IEEE} Trans. Syst., Man, Cybern. {A}
{IEEE} Transactions on Systems, Man, and Cybernetics---Part {B}: Cybernetics = {IEEE} Trans. Syst., Man, Cybern. {B}
{IEEE} Transactions on Systems, Man, and Cybernetics---Part {C}: Applications and Reviews = {IEEE} Trans. Syst., Man, Cybern. {C}
{IEEE} Transactions on Systems, Man, and Cybernetics: Systems = {IEEE} Trans. Syst., Man, Cybern., Syst.
{IEEE} Transactions on Terahertz Science and Technology = {IEEE} Trans. {THz} Sci. Technol.
{IEEE} Transactions on Transportation Electrification = {IEEE} Trans. Transport. Electrific.
{IEEE} Transactions on Ultrasonics Engineering = {IEEE} Trans. Ultrason. Eng.
{IEEE} Transactions on Ultrasonics, Ferroelectrics, and Frequency Control = {IEEE} Trans. Ultrason., Ferroelectr., Freq. Control
{IEEE} Transactions on Vehicular Communications = {IEEE} Trans. Veh. Commun.
{IEEE} Transactions on Vehicular Technology = {IEEE} Trans. Veh. Technol.
{IEEE} Transactions on Very Large Scale Integration ({VLSI}) Systems = {IEEE} Trans. {VLSI} Syst.
{IEEE} Transactions on Visualization and Computer Graphics = {IEEE} Trans. Vis. Comput. Graphics
{IEEE} Transactions on Wireless Communications = {IEEE} Trans. Wireless Commun.
{IEEE} Translation Journal on Magnetics in Japan = {IEEE} Transl. J. Magn. Jpn.
{IEEE} Vehicular Technology Magazine = {IEEE} Veh. Technol. Mag.
{IEEE} Wireless Communications = {IEEE} Wireless Commun.
{IEEE} Wireless Communications Letters = {IEEE} Wireless Commun. Lett.
{IEEE} {ASSP} Magazine = {IEEE} {ASSP} Mag.
{IEEE} {IT} Professional = {IEEE} {IT} Prof.
{IEEE} {RFIC} Journal = {IEEE} {RFIC} J.
{IEEE} {RFID} Journal = {IEEE} {RFID} J.
"""

def parse_citation_guides():
    url = 'http://librarybestbets.fairfield.edu/citationguides/ieee#AbbreviationsforIEEEPublications'
    import bs4
    import requests
    r  = requests.get(url)
    data = r.text
    soup = bs4.BeautifulSoup(data, 'html.parser')
    table = soup.findAll('table')[-1]

    def parse_bs_table(table):
        n_columns = 0
        n_rows = 0
        column_names = []
        # Find number of rows and columns
        # we also find the column titles if we can
        for row in table.find_all('tr'):
            td_tags = row.find_all('td')
            if len(td_tags) > 0:
                n_rows += 1
                if n_columns == 0:
                    n_columns = len(td_tags)
            # Handle column names if we find them
            th_tags = row.find_all('th')
            if len(th_tags) > 0 and len(column_names) == 0:
                for th in th_tags:
                    column_names.append(th.get_text())

        # Safeguard on Column Titles
        if len(column_names) > 0 and len(column_names) != n_columns:
            raise Exception("Column titles do not match the number of columns")
        columns = column_names if len(column_names) > 0 else range(0, n_columns)
        df = pd.DataFrame(columns=columns, index=list(range(0, n_rows)))
        row_marker = 0
        for row in table.find_all('tr'):
            column_marker = 0
            columns = row.find_all('td')
            for column in columns:
                df.iat[row_marker, column_marker] = column.get_text().strip()
                column_marker += 1
            if len(columns) > 0:
                row_marker += 1
        return df
    df = parse_bs_table(table)

    last = None
    data = []
    for record in df.to_dict(orient='records'):
        if record['Publication']:
            last = record
            data.append(record)
        else:
            if 'acc_alias' not in last:
                last['acc_alias'] = []
                last['abbrev_alias'] = []
            last['acc_alias'].append(record['Acronym'])
            last['abbrev_alias'].append(record['Reference Abbreviation'])
    print(ut.repr2(data, nl=2))

[
    {
        'Acronym': 'AES',
        'Publication': 'IEE Transactions on Aerospace and Electronic Systems',
        'Reference Abbreviation': 'IEEE Trans. Aerosp. Electron. Syst.',
        'abbrev_alias': ['IEEE Trans. Aeronaut. Navig. Electron.*', 'IEEE Trans. Aerosp. Navig. Electron.*', 'IEEE Trans. Aerosp.', 'IEEE Trans. Mil. Electron.*', 'IEEE Trans. Airborne Electron.'],
        'acc_alias': ['ANE*', 'ANE*', 'AS*', 'MIL*', 'AE*'],
    },
    {
        'Acronym': 'AP',
        'Publication': 'IEEE Transactions on Antennas and Propagation',
        'Reference Abbreviation': 'IEEE Trans. Antennas Propag.',
    },
    {
        'Acronym': 'LAWP',
        'Publication': 'IEEE Antennas and Wireless Propagation Letters',
        'Reference Abbreviation': 'IEEE Antennas Wireless Propag. Lett.',
    },
    {
        'Acronym': 'ASC',
        'Publication': 'IEEE Transactions on Applied Superconductivity',
        'Reference Abbreviation': 'IEEE Trans. Appl. Supercond.',
    },
    {
        'Acronym': 'ASLP',
        'Publication': 'IEEE/ACM Transactions on Audio, Speech, and Language Processing',
        'Reference Abbreviation': 'IEEE/ACM Trans. Audio, Speech, Language Process.',
    },
    {
        'Acronym': 'ASL',
        'Publication': 'IEEE Transactions on Audio, Speech, and Language Processing',
        'Reference Abbreviation': 'IEEE Trans. Audio, Speech, Language Process. (2006-2013)',
        'abbrev_alias': ['IEEE Speech Audio Process. (1993-2005)'],
        'acc_alias': ['SAP*'],
    },
    {
        'Acronym': 'AC',
        'Publication': 'IEEE Transactions on Automatic Control',
        'Reference Abbreviation': 'IEEE Trans. Autom. Control',
    },
    {
        'Acronym': 'ASE',
        'Publication': 'IEEE Transactions on Automation Science and Engineering',
        'Reference Abbreviation': 'IEEE Trans. Autom. Sci. Eng. (from July 2004)',
    },
    {
        'Acronym': 'AMD',
        'Publication': 'IEEE Transactions on Autonomous Mental Development',
        'Reference Abbreviation': 'IEEE Trans. Auton. Mental Develop.',
    },
    {
        'Acronym': 'BD',
        'Publication': 'IEEE Transactions on Big Data',
        'Reference Abbreviation': 'IEEE Trans. Big Data',
    },
    {
        'Acronym': 'BHI',
        'Publication': 'IEEE Journal of Biomedical and Health Informatics',
        'Reference Abbreviation': 'IEEE J. Biomed Health Inform.',
        'abbrev_alias': ['IEEE Trans. Inf. Technol. Biomed. (1995-2012)'],
        'acc_alias': ['ITB'],
    },
    {
        'Acronym': 'BCAS',
        'Publication': 'IEEE Transactions on Biomedical Circuits and Systems',
        'Reference Abbreviation': 'IEEE Trans. Biomed. Circuits Syst.',
    },
    {
        'Acronym': 'RBME',
        'Publication': 'IEE Reviews in Biomedical Engineering',
        'Reference Abbreviation': 'IEE Rev. Biomed. Eng.',
    },
    {
        'Acronym': 'BME',
        'Publication': 'IEE Transactions on Biomedical Engineering',
        'Reference Abbreviation': 'IEEE Trans. Biomed. Eng.',
        'abbrev_alias': ['IEE Trans Bio-Med Eng.*', 'IEEE Trans. Bio-Med. Electron.*', 'IEEE Trans. Med. Electron.'],
        'acc_alias': ['BME*', 'BME*', 'PGME*'],
    },
    {
        'Acronym': 'BC',
        'Publication': 'IEEE Transactions on Broadcasting',
        'Reference Abbreviation': 'IEEE Trans. Broadcast.',
    },
    {
        'Acronym': 'CSI',
        'Publication': 'IEEE Transactions on Circuits and Systems- I: Regular Papers',
        'Reference Abbreviation': 'IEEE Trans. Circuits Syst. I, Reg. Papers',
    },
    {
        'Acronym': 'CSII',
        'Publication': 'IEEE Transactions on Circuits and Systems-II: Express Briefs',
        'Reference Abbreviation': 'IEEE Trans. Circuits Syst. II, Exp. Briefs',
    },
    {
        'Acronym': 'CAS1*',
        'Publication': 'IEEE Transactions on Circuits and Systems-I: Fundamental Theory and Applications',
        'Reference Abbreviation': 'IEEE Trans. Circuits Syst. I Fundam. Theory Appl. (1993-2003)',
    },
    {
        'Acronym': 'CAS2*',
        'Publication': 'IEEE Transactions on Circuits and Systems-II: Analog and Digital Signal Processing',
        'Reference Abbreviation': 'IEEE Trans. Circuits Syst. II, Analog Digit. Signal Process (1993-2003)',
        'abbrev_alias': ['IEEE Trans. Circuits Syst.* (1974-1992)', 'IEEE Trans. Circuit Theory* (until 1973)'],
        'acc_alias': ['CAS*', 'CT*'],
    },
    {
        'Acronym': 'CSVT',
        'Publication': 'IEEE Transactions on Circuits and Systems for Video Technology',
        'Reference Abbreviation': 'IEEE Trans. Circuits Syst. Video Technol.',
    },
    {
        'Acronym': 'CC',
        'Publication': 'IEEE Transactions on Cloud Computing',
        'Reference Abbreviation': 'IEEE Trans. Cloud Comput.',
    },
    {
        'Acronym': 'CCN',
        'Publication': 'IEEE Transactions on Cognitive Communications and Networking',
        'Reference Abbreviation': 'IEEE Trans. Cogn. Commun. Netw.',
    },
    {
        'Acronym': 'COM',
        'Publication': 'IEEE Transactions on Communications',
        'Reference Abbreviation': 'IEEE Trans. Commun.',
        'abbrev_alias': ['IEEE Trans. Commun. Technol.* (until 1971)'],
        'acc_alias': ['COM*'],
    },
    {
        'Acronym': 'COMML',
        'Publication': 'IEEE Communication Letters',
        'Reference Abbreviation': 'IEEE\xa0Commun. Lett.',
    },
    {
        'Acronym': 'COMST',
        'Publication': 'IEEE Communications Surveys & Tutorials',
        'Reference Abbreviation': 'IEEE\xa0Commun. Surveys Tuts.',
    },
    {
        'Acronym': 'CPMT',
        'Publication': 'IEEE Transactions on Components, Packaging and Manufacturing Technology',
        'Reference Abbreviation': 'IEEE Trans. Compon. Packag. Manuf. Technol.',
        'abbrev_alias': ['IEEE Trans. Compon. Packag. Technol. (1999-2010)', 'IEEE Trans. Compon., Packag., Manuf. Technol. A (1994-1998)', 'IEEE Trans. Compon., Hybrids, Manuf. Technol.* (1978-1993)', 'IEEE Trans. Manuf. Technol.* (1972-1977)', 'IEEE Trans. Parts, Hybrids, Packag.* (June 1971-1977)', 'IEEE Trans. Parts, Mater., Packag* (1965-1971)', 'IEEE Trans. Adv. Packag. (1999-2010)', 'IEEE Trans. Compon., Packag., Manuf. Technol. B (1994-1998)', 'IEEE Trans. Electron. Packag. Manuf. (1999-2010)', 'IEEE Trans. Compon., Packag., Manuf. Technol. C (1996-1998)'],
        'acc_alias': ['CAPT', 'CPMTA', 'CHMT*', 'MFT*', 'PHP*', 'PMP*', 'ADVP', 'CPMTB', 'EPM', 'CPMTC'],
    },
    {
        'Acronym': 'CBB',
        'Publication': 'IEEE/ACM Transactions on Computational Biology and Bioinformatics',
        'Reference Abbreviation': 'IEEE/ACM Trans. Comput. Biol. Bioinf.',
    },
    {
        'Acronym': 'CIAIG',
        'Publication': 'IEEE Transactions on Computational Intelligence and AI in Games',
        'Reference Abbreviation': 'IEEE Trans. Comput. Intell. AI in Games',
    },
    {
        'Acronym': 'CSS',
        'Publication': 'IEEE Transactions on Computational Social Systems',
        'Reference Abbreviation': 'IEEE Trans. Comput. Social Syst.',
    },
    {
        'Acronym': 'C',
        'Publication': 'IEEE Transactions on Computers',
        'Reference Abbreviation': 'IEEE Trans. Comput.',
    },
    {
        'Acronym': 'CAD',
        'Publication': 'IEEE Transactions on Computer-Aided Design of Integrated Circuits and Systems',
        'Reference Abbreviation': 'IEEE Trans. Comput.-Aided Design Integr. Circuits Syst.',
    },
    {
        'Acronym': 'CAL',
        'Publication': 'IEEE Computer Architectural Letters',
        'Reference Abbreviation': 'IEEE Comput. Archit. Lett.',
    },
    {
        'Acronym': 'CE',
        'Publication': 'IEEE Transactions on Consumer Electronics',
        'Reference Abbreviation': 'IEEE Trans. Consum. Electron.',
        'abbrev_alias': ['IEEE Trans. Broadcast. Telev. Receiv. (1963-1974)'],
        'acc_alias': ['BTR'],
    },
    {
        'Acronym': 'CST',
        'Publication': 'IEEE Transactions on Control Systems Technology',
        'Reference Abbreviation': 'IEEE Trans. Control Syst. Technol.',
        'abbrev_alias': ['IEEE Trans. Cybern.'],
        'acc_alias': ['CYB'],
    },
    {
        'Acronym': 'SMCB*',
        'Publication': 'IEEE Transactions on Cybernetics',
        'Reference Abbreviation': 'IEEE Trans. Syst. Man, Cybern. B, Cybern. (1995-2012 )',
    },
    {
        'Acronym': 'DMR',
        'Publication': 'IEEE Transactions on Device and Materials Liability',
        'Reference Abbreviation': 'IEEE Trans. Device Mater. Rel.',
    },
    {
        'Acronym': 'DEI',
        'Publication': 'IEEE Transactions on Dielectrics and Electrical Insulation',
        'Reference Abbreviation': 'IEEE Trans. Dielectr. Electr. Insul.',
        'abbrev_alias': ['IEEE Trans.\xa0Electr. Insul.* (through 1993)'],
        'acc_alias': ['EI*'],
    },
    {
        'Acronym': 'DT',
        'Publication': 'IEEE/OSA Journal of Display Technology',
        'Reference Abbreviation': 'J Display Technol.',
    },
    {
        'Acronym': 'E',
        'Publication': 'IEEE Transactions on Education',
        'Reference Abbreviation': 'IEEE Trans. Educ.',
    },
    {
        'Acronym': 'EMC',
        'Publication': 'IEEE Transactions on Electromagnetic Compatibility',
        'Reference Abbreviation': 'IEEE Trans. Electromagn. Compat.',
        'abbrev_alias': ['IEEE Trans. Radio Freq. Interference*'],
        'acc_alias': ['RFI*'],
    },
    {
        'Acronym': 'ED',
        'Publication': 'IEEE Transactions on Electron Devices',
        'Reference Abbreviation': 'IEEE Trans.Electron Devices',
    },
    {
        'Acronym': 'EDS',
        'Publication': 'IEEE Journal of Electron Devices Society',
        'Reference Abbreviation': 'IEEE J. Electron Devices Soc.',
    },
    {
        'Acronym': 'EDL',
        'Publication': 'IEEE Electron Device Letters',
        'Reference Abbreviation': 'IEEE Electron Device Lett.',
    },
    {
        'Acronym': 'EPM',
        'Publication': 'IEEE Transactions on Electronics Packaging Manufacturing',
        'Reference Abbreviation': 'IEEE Trans. Electron. Packag. Manuf. (1999-2010)',
    },
    {
        'Acronym': 'ES',
        'Publication': 'IEEE Embedded Systems Letters',
        'Reference Abbreviation': 'IEEE Embedded Syst. Lett.',
    },
    {
        'Acronym': 'ETC',
        'Publication': 'IEEE Transactions on Emerging Topics in Computing',
        'Reference Abbreviation': 'IEEE Trans. Emerg. Topics. Comput.',
    },
    {
        'Acronym': 'ETCAS',
        'Publication': 'IEEE Journal of Emerging and Selected Topics in Circuits and Systems',
        'Reference Abbreviation': 'IEEE J. Emerg. Sel. Topics Circuits Syst.',
    },
    {
        'Acronym': 'ESTPE',
        'Publication': 'IEEE Journal on Emerging and Selected Topics in Power Electronics',
        'Reference Abbreviation': 'IEEE J. Emerg. Sel. Topics Power Electron.',
    },
    {
        'Acronym': 'EC',
        'Publication': 'IEEE Transactions on Energy Conversion',
        'Reference Abbreviation': 'IEEE Trans. Energy Convers.',
    },
    {
        'Acronym': 'EM',
        'Publication': 'IEEE Transactions on Engineering Management',
        'Reference Abbreviation': 'IEEE Trans. Eng. Manag.',
    },
    {
        'Acronym': 'EVC',
        'Publication': 'IEEE Transactions on Evolutionary Computation',
        'Reference Abbreviation': 'IEEE Trans. Evol. Comput.',
    },
    {
        'Acronym': 'XCDC',
        'Publication': 'IEEE Transactions on Exploratory Solid-State Computational Devices and Circuits',
        'Reference Abbreviation': 'IEEE Trans. Explor. Solid-State Comput. Devices Circuits',
    },
    {
        'Acronym': 'FUZZ',
        'Publication': 'IEEE Transactions on Fuzzy Systems',
        'Reference Abbreviation': 'IEEE Trans. Fuzzy Syst.',
    },
    {
        'Acronym': 'GRS',
        'Publication': 'IEEE Transaction on Geoscience and Remote Sensing',
        'Reference Abbreviation': 'IEEE Trans. Geosci. Remote Sens.',
    },
    {
        'Acronym': 'GRSL',
        'Publication': 'IEEE Geoscience and Remote Sensing Letters',
        'Reference Abbreviation': 'IEEE Geosci. Remote Ses. Lett.',
    },
    {
        'Acronym': 'HMS*',
        'Publication': 'IEEE Transctions on Human-Machine Systems',
        'Reference Abbreviation': 'IEEE Trans. Human-Mach. Syst.',
        'abbrev_alias': ['IEEE Trans. Syst., Man, Cybern. C, Appl. Rev.(1995-2012)', 'IEEE Trans. Syst., Man, Cybern.* (1971-1995)', 'IEEE Trans. Syst. Sci. Cybern.* (through 1970)'],
        'acc_alias': ['SMCC*', 'SMC*', 'SSC*'],
    },
    {
        'Acronym': 'IP',
        'Publication': 'IEEE Transactions on Image Processing',
        'Reference Abbreviation': 'IEEE Trans. Image Process.',
    },
    {
        'Acronym': 'IE',
        'Publication': 'IEEE Transactions on Industrial Electronics',
        'Reference Abbreviation': 'IEEE Trans. Ind. Electron.',
    },
    {
        'Acronym': 'II',
        'Publication': 'IEEE Transactions on Industrial Informatics',
        'Reference Abbreviation': 'IEEE Trans. Ind. Informat.',
    },
    {
        'Acronym': 'IA',
        'Publication': 'IEEE Transactions on on Industry Applications',
        'Reference Abbreviation': 'IEEE Trans. Ind. Appl.',
    },
    {
        'Acronym': 'IFS',
        'Publication': 'IEEE Transactions on Information Forensics and Security',
        'Reference Abbreviation': 'IEEE Trans. Inf. Forensics Security',
    },
    {
        'Acronym': 'IT',
        'Publication': 'IEEE Transactions on Information Theory',
        'Reference Abbreviation': 'IEEE Trans. Inf. Theory',
    },
    {
        'Acronym': 'IM',
        'Publication': 'IEEE Transactions on Instrumentation and Measurement',
        'Reference Abbreviation': 'IEEE Trans. Instrum. Meas.',
        'abbrev_alias': ['IEEE Trans. Instrum.*'],
        'acc_alias': ['I, PGI*'],
    },
    {
        'Acronym': 'ITS',
        'Publication': 'IEEE Transactions of Intelligent Transportation Systems',
        'Reference Abbreviation': 'IEEE Trans. Intell. Transp. Syst.',
    },
    {
        'Acronym': 'IoT',
        'Publication': 'IEEE Internet of Things Journal',
        'Reference Abbreviation': 'IEEE Internet Things J.',
    },
    {
        'Acronym': 'KDE',
        'Publication': 'IEEE Transactions on Knowledge and Data Engineering',
        'Reference Abbreviation': 'IEEE Trans. Knowl. Data. Eng.',
    },
    {
        'Acronym': 'LS',
        'Publication': 'IEEE Life Sciences Letters',
        'Reference Abbreviation': 'IEEE Life Sci. Lett.',
    },
    {
        'Acronym': 'LT',
        'Publication': 'IEEE/OSA Journal of Lightwave Technology',
        'Reference Abbreviation': 'J. Lightw. Technol.',
    },
    {
        'Acronym': 'MAG',
        'Publication': 'IEEE Transactions on Magnetics',
        'Reference Abbreviation': 'IEEE Trans. Magn.',
    },
    {
        'Acronym': 'MAGL',
        'Publication': 'IEEE Magnetics Letters',
        'Reference Abbreviation': 'IEEE Magn. Lett.',
    },
    {
        'Acronym': 'MECH',
        'Publication': 'IEEE/ASME Transactions on Mechatronics',
        'Reference Abbreviation': 'IEEE/ASME Trans. Mechatronics',
    },
    {
        'Acronym': 'MI',
        'Publication': 'IEEE Transactions on Medical Imaging',
        'Reference Abbreviation': 'IEEE Trans. Med. Imag.',
    },
    {
        'Acronym': 'MEMS',
        'Publication': 'IEEE Journal of Microelectromechanical Systems',
        'Reference Abbreviation': 'J. Microelectromech. Syst.',
    },
    {
        'Acronym': 'MEMS',
        'Publication': 'IEEE/ASME Journal of Microelectromechanical Systems',
        'Reference Abbreviation': 'J. Microelectromech. Syst. (1992-2013)',
    },
    {
        'Acronym': 'MWCL',
        'Publication': 'IEEE Microwave and Wireless Components Letters',
        'Reference Abbreviation': 'IEEE\xa0 Microw. Compon. Lett.',
        'abbrev_alias': ['IEEE Microw. Guided Wave Lett. (1991-2000)'],
        'acc_alias': ['MGWL'],
    },
    {
        'Acronym': 'MTT',
        'Publication': 'IEEE Transactions on Microwave Theory and Techniques',
        'Reference Abbreviation': 'IEEE Trans. Microw. Theory Techn.',
    },
    {
        'Acronym': 'MC',
        'Publication': 'IEEE Transactions on Mobile Computing',
        'Reference Abbreviation': 'IEEE Trans. Mobile Comput.',
    },
    {
        'Acronym': '',
        'Publication': 'IEEE Transactions on Molecular, Biological and Multiscale Communications',
        'Reference Abbreviation': 'IEEE Trans. Mol. Biol. Multi-Scale Commun.',
    },
    {
        'Acronym': 'MM',
        'Publication': 'IEEE Transactions on Multimedia',
        'Reference Abbreviation': 'IEEE Trans. Multimedia',
    },
    {
        'Acronym': '',
        'Publication': 'IEEE Transactions on Multi-Scale Computing',
        'Reference Abbreviation': 'IEEE Trans. Multi-Scale Comput. Syst.',
    },
    {
        'Acronym': 'NB',
        'Publication': 'IEEE Transactions on NanoBioscience',
        'Reference Abbreviation': 'IEEE Trans. Nanobiosci.',
    },
    {
        'Acronym': 'NANO',
        'Publication': 'IEEE Transactions on Nanotechnology',
        'Reference Abbreviation': 'IEEE Trans. Nanotechnol.',
    },
    {
        'Acronym': 'ENANO',
        'Publication': 'IEEE Nanotechnology Express',
        'Reference Abbreviation': 'IEEE Nanotechnol. Express',
    },
    {
        'Acronym': 'NET',
        'Publication': 'IEEE/ACM Transactions on Networking',
        'Reference Abbreviation': 'IEEE/ACM Trans. Netw.',
    },
    {
        'Acronym': 'NNLS',
        'Publication': 'IEEE Transactions on Neural Networks and Learning Systems',
        'Reference Abbreviation': 'IEEE Tans. Neural Netw. Learn Syst.',
        'abbrev_alias': ['IEEE Trans. Neural Netw. (1990-2011)'],
        'acc_alias': ['NN'],
    },
    {
        'Acronym': 'NS',
        'Publication': 'IEEE Transactions on Nuclear Science',
        'Reference Abbreviation': 'IEEE Trans. Nucl. Sci.',
    },
    {
        'Acronym': 'NSRE',
        'Publication': 'IEEE Transactions on Neural Systems and Rehabilitation Engineering',
        'Reference Abbreviation': 'IEEE Trans. Neural Syst. Rehabil. Eng.',
        'abbrev_alias': ['IEEE Trans. Rehabil. Eng.* (1993-2000)'],
        'acc_alias': ['RE*'],
    },
    {
        'Acronym': 'OE',
        'Publication': 'IEEE Journal of Oceanic Engineering',
        'Reference Abbreviation': 'IEEE J. Ocean. Eng.',
    },
    {
        'Acronym': 'OCN',
        'Publication': 'IEEE Journal of Optical Communications and Networking',
        'Reference Abbreviation': 'IEEE J. Opt. Commun. Netw.',
    },
    {
        'Acronym': 'PDS',
        'Publication': 'IEEE Transactions on Parallel and Distributed Systems',
        'Reference Abbreviation': 'IEEE Trans. Parallel Distrib. Syst.',
    },
    {
        'Acronym': 'PAMI',
        'Publication': 'IEEE Transactions on Pattern Analysis and Machine Intelligence',
        'Reference Abbreviation': 'IEEE Trans. Pattern Anal. Mach. Intell.',
    },
    {
        'Acronym': 'PTL',
        'Publication': 'IEEE Photonics Technology Letters',
        'Reference Abbreviation': 'IEEE Photon. Technol. Lett.',
    },
    {
        'Acronym': 'PJ',
        'Publication': 'IEEE Photonics Journal',
        'Reference Abbreviation': 'IEEE Photon. J',
    },
    {
        'Acronym': 'PHOT',
        'Publication': 'IEEE Journal of Photovoltaics',
        'Reference Abbreviation': 'IEEE J. Photovolt.',
    },
    {
        'Acronym': 'PS',
        'Publication': 'IEEE Transactions on Plasma Science',
        'Reference Abbreviation': 'IEEE Trans. Plasma Sci.',
    },
    {
        'Acronym': 'PAS*',
        'Publication': 'IEEE Transactions on Power Apparatus and Systems',
        'Reference Abbreviation': 'IEEE Trans. Power App. Syst.* (through 1985)',
    },
    {
        'Acronym': 'PWRD',
        'Publication': 'IEEE Transactions on Power Delivery',
        'Reference Abbreviation': 'IEEE Trans. Power Del.',
    },
    {
        'Acronym': 'PEL',
        'Publication': 'IEEE Transactions on Power Electronics',
        'Reference Abbreviation': 'IEEE Trans. Power Electron.',
        'abbrev_alias': ['IEEE Power Electron. Lett. (2003-2005; abolished)'],
        'acc_alias': ['LPEL'],
    },
    {
        'Acronym': 'PWRS',
        'Publication': 'IEEE Transactions on Power Systems',
        'Reference Abbreviation': 'IEEE Trans. Power Syst.',
    },
    {
        'Acronym': 'PSE',
        'Publication': 'IEEE Journal of Product Safety Engineering',
        'Reference Abbreviation': 'IEEE J. Product Safety Eng.',
    },
    {
        'Acronym': 'PETS',
        'Publication': 'IEEE Power and Energy Technology Systems Journal',
        'Reference Abbreviation': 'IEEE Power Energy Technol. Syst. J.',
    },
    {
        'Acronym': 'PC',
        'Publication': 'IEEE Transactions on Professional Communication',
        'Reference Abbreviation': 'IEEE Trans. Prof. Commun.',
    },
    {
        'Acronym': 'QE',
        'Publication': 'IEEE Journal of Quantum Electronics',
        'Reference Abbreviation': 'IEEE J. Quantum Electron.',
    },
    {
        'Acronym': 'RFIC',
        'Publication': 'IEEE RFIC Journal',
        'Reference Abbreviation': 'IEEE RFIC J.',
    },
    {
        'Acronym': 'RFID',
        'Publication': 'IEEE RFID Journal',
        'Reference Abbreviation': 'IEEE RFID J.',
    },
    {
        'Acronym': 'R',
        'Publication': 'IEEE Transactions on Reliability',
        'Reference Abbreviation': 'IEEE Trans. Rel.',
    },
    {
        'Acronym': 'RO',
        'Publication': 'IEEE Transactions on Robotics',
        'Reference Abbreviation': 'IEEE Trans. Robot.',
    },
    {
        'Acronym': 'RA*',
        'Publication': 'IEEE Transactions on Robotics and Automation',
        'Reference Abbreviation': 'IEEE Trans. Robot. Autom.(1989-Jun.2004)',
        'abbrev_alias': ['IEEE J. Robot. Autom.*(1985-1988)'],
        'acc_alias': ['RA*'],
    },
    {
        'Acronym': 'STARS',
        'Publication': 'IEEE Journal of Selected Topics in Applied Earth Observations and Remote Sensing',
        'Reference Abbreviation': 'IEEE J. Sel. Topics Appl. Earth Observ. Remote Sens.',
    },
    {
        'Acronym': 'SAC',
        'Publication': 'IEEE Journal on Selected Areas in Communications',
        'Reference Abbreviation': 'IEEE J. Sel. Areas Commun.',
    },
    {
        'Acronym': 'STQE',
        'Publication': 'IEEE Journal of Selected Topics in Quantum Electronics',
        'Reference Abbreviation': 'IEEE J. Sel. Topics Quantum Electron.',
    },
    {
        'Acronym': 'STSP',
        'Publication': 'IEEE Journal of Selected Topics in Signal Processing',
        'Reference Abbreviation': 'IEEE J. Sel. Topics Signal Process.',
    },
    {
        'Acronym': 'SM',
        'Publication': 'IEEE Transactions on Semiconductor Manufacturing',
        'Reference Abbreviation': 'IEEE Trans. Semicond. Manuf.',
    },
    {
        'Acronym': 'SEN',
        'Publication': 'IEEE Sensors Journal',
        'Reference Abbreviation': 'IEEE Sensors J.',
    },
    {
        'Acronym': 'SP',
        'Publication': 'IEEE Transactions on Signal Processing',
        'Reference Abbreviation': 'IEEE Trans. Signal Process.',
        'abbrev_alias': ['IEEE Trans. Acoust., Speech, Signal Process.*(1975-1990)',
                         'IEE Trans. Audio Electroacoust. (until 1974)'],
        'acc_alias': ['ASSP*', 'AU*'],
    },
    {
        'Acronym': 'SPL',
        'Publication': 'IEEE Signal Processing Letters',
        'Reference Abbreviation': 'IEEE Signal Process. Lett.',
    },
    {
        'Acronym': 'SG',
        'Publication': 'IEEE Transactions on Smart Grid',
        'Reference Abbreviation': 'IEEE Trans. Smart Grid',
    },
    {
        'Acronym': 'STE',
        'Publication': 'IEEE Transactions on Sustainable Energy',
        'Reference Abbreviation': 'IEEE Trans. Sustain. Energy',
    },
    {
        'Acronym': 'SJ',
        'Publication': 'IEEE Systems Journal',
        'Reference Abbreviation': 'IEEE Syst. J.',
    },
    {
        'Acronym': 'SE',
        'Publication': 'IEEE Transactions on Software Engineering',
        'Reference Abbreviation': 'IEEE Trans. Softw. Eng.',
    },
    {
        'Acronym': 'SSC',
        'Publication': 'IEEE Journal of Solid-State Circuits',
        'Reference Abbreviation': 'J. Solid-State Circuits',
    },
    {
        'Acronym': 'SMC',
        'Publication': 'IEEE Transactions on Systems, Man, and Cybernetics: Systems',
        'Reference Abbreviation': 'IEEE Trans. Syst., Man, Cybern., Syst.',
        'abbrev_alias': ['IEEE Trans. Syst., Man, Cybern. A, Syst., Humans (1995-2012)',
                         'IEEE Trans. Man-Mach. Syst.* (through 1970)',
                         'Hum. Factors Electron.* (through 1968)'],
        'acc_alias': ['SMCA*', 'MMS*', 'HFE*'],
    },
    {
        'Acronym': 'TEHM',
        'Publication': 'IEEE Journal of Translational Engineering in Health and Medicine',
        'Reference Abbreviation': 'IEEE J. Transl. Eng. Health Med.',
    },
    {
        'Acronym': 'TJMJ',
        'Publication': 'IEEE Translation Journal on Magnetics in Japan',
        'Reference Abbreviation': 'IEEE Transl. J. Magn. Jpn. (through 2010)',
    },
    {
        'Acronym': 'TCAD',
        'Publication': 'IEEE Journal on Technology in Computer Aided Design',
        'Reference Abbreviation': 'IEEE J. Technol. Computer Aided Des.',
    },
    {
        'Acronym': 'THz',
        'Publication': 'IEEE Transactions on Terahertz Science and Technology',
        'Reference Abbreviation': 'IEEE Trans. THz Sci. Technol.',
    },
    {
        'Acronym': '',
        'Publication': 'IEEE Transactions on Transportation Electrification',
        'Reference Abbreviation': 'IEEE Trans. Transport. Electrific.',
    },
    {
        'Acronym': 'UFFC',
        'Publication': 'IEEE Transactions on Ultrasonics, Ferroelectrics, and Frequency Control',
        'Reference Abbreviation': 'IEEE Trans. Ultrason., Ferroelectr., Freq. Control',
        'abbrev_alias': ['IEEE Trans. Sonics Ultrason.* (through 1985)', 'IEEE Trans. Ultrason. Eng.*', 'IEEE Trans. Ultrason. Eng.*'],
        'acc_alias': ['SU*', 'UE*', 'PGUE*'],
    },
    {
        'Acronym': 'VT',
        'Publication': 'IEEE Transactions on\xa0 Vehicular Technology',
        'Reference Abbreviation': 'IEEE Trans.\xa0 Veh. Technol.',
        'abbrev_alias': ['IEEE Trans. Veh. Commun.*'],
        'acc_alias': ['VC*'],
    },
    {
        'Acronym': 'VLSI',
        'Publication': 'IEEE Transactions on Very Large Scale Integration (VLSI) Systems',
        'Reference Abbreviation': 'IEEE Trans.\xa0 Very Large Scale Integr. (VLSI) Syst.',
    },
    {
        'Acronym': 'VCG',
        'Publication': 'IEEE Transactions on Visualization and Computer Graphics',
        'Reference Abbreviation': 'IEEE Trans. Vis. Comput. Graphics',
    },
    {
        'Acronym': 'WC',
        'Publication': 'IEEE Transactions on Wireless Communications',
        'Reference Abbreviation': 'IEEE Trans. Wireless Commun.',
    },
    {
        'Acronym': '',
        'Publication': 'Proceedings of the IEEE',
        'Reference Abbreviation': 'Proc. IEEE',
        'abbrev_alias': ['Proc. IRE* (through 1962)'],
        'acc_alias': [''],
    },
]

lines = [line.strip() for line in JabRef_list.splitlines() if line.strip()]
full_to_short = {}
for line in lines:
    line = line.replace('{', '').replace('}', '')
    fullname, shortname = (p.strip() for p in line.split('='))
    assert fullname not in full_to_short
    full_to_short[fullname] = shortname
